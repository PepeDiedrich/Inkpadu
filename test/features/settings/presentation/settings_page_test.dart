...
void main() {
  ...
  group('SettingsPage', () {
    ...
    testWidgets('does not render logout section when logged out', (tester) async {
      when(() => mockAuthController.isLoggedIn).thenReturn(false);
      tester.view.physicalSize = const Size(1080, 4000);
      addTearDown(tester.view.resetPhysicalSize);
    ...
    testWidgets('navigates to Input Devices settings', (tester) async {
      ...
      // It should have "Mouse" (based on PointerSettingsScope content analysis or similar)
      // or "Touch"
      // Let's verify that we are indeed on a new page by checking something specific from PointerSettingsPage if known,
      // or just that the previous page content is gone.
    });
